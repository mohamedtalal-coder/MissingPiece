  ...
  const [orders, setOrders] = useState([
    {
      ...
      items: [{ title: 'Midnight Nebula Luxury Puzzle', qty: 1, price: 129.00 }]
    },
    {
      id: 'ORD-8410',
      date: '2026-05-20',
      ...
      items: [{ title: 'Golden Cosmos Masterpiece', qty: 1, price: 210.00 }]
    }
  ]);
  ...
  return (
    <div className="max-w-5xl mx-auto px-8 py-12 font-sans space-y-8">
      <div className="flex items-center gap-3 border-b border-[#7e22ce]/30 pb-6">
        <div className="w-10 h-10 rounded-xl bg-[#130e21] border border-[#7e22ce]/50 flex items-center justify-center shadow-[0_0_15px_rgba(126,34,206,0.3)]">
          <Package className="w-5 h-5 text-[#c084fc]" />
        </div>
        <div>
          <h1 className="text-2xl font-serif font-bold text-white">Order History</h1>
          <p className="text-xs text-[#cbd5e1]">Track your past and active luxury puzzle orders</p>
        </div>
      </div>

      <div className="space-y-4">
        {orders.map((order) => (
          <div key={order.id} className="bg-[#130e21] border border-[#7e22ce]/40 rounded-2xl p-6 space-y-4 shadow-[0_0_20px_rgba(126,34,206,0.15)]">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#7e22ce]/20 pb-4">
              <div>
                <span className="text-xs text-[#c084fc] font-semibold">{order.id}</span>
                <p className="text-[11px] text-[#cbd5e1]">Placed on: {order.date}</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-[#cbd5e1]">
              <div className="space-y-1">
                <p className="text-white font-bold">Shipping Address:</p>
                <p>{order.address}</p>
              </div>
              <div className="space-y-1 md:text-right">
                <p className="text-white font-bold">Total Amount:</p>
                <p className="text-base text-[#c084fc] font-bold">${order.total.toFixed(2)}</p>
              </div>
            </div>
          </div>
        ))}
