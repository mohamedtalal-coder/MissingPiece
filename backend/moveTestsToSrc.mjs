...
for (const oldPath of tests) {
  const newPath = oldPath.replace('tests', 'src/tests');
...
tsconfig = tsconfig.replace('"include": ["src", "tests"]', '"include": ["src"]');
fs.writeFileSync(tsconfigPath, tsconfig);

console.log("Moved " + tests.length + " test files to src/tests.");
